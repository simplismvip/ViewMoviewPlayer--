...
- (void)setupVideoByPath:(NSString *)path page:(NSInteger)page
{
    ...
    if (local.count > 0) {
        ...
        [self setupVideoByArray:local];
        
    } else if (url.count > 0){
    
        [self setupVideoByArray:url];
    }
...
// 根据路径初始化播放器
- (void)setupVideoByArray:(NSMutableArray *)array
{
    // 拿到文件路径
...
- (void)creatBtn
{
    ...
    share.frame = CGRectMake(self.view.frame.size.width - 50, 0, 60, 50);
    ...
    play.center = CGPointMake(self.view.frame.size.width/2, self.view.frame.size.height/2);
    ...
    UIView *sliderView = [[UIView alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height - height, self.view.frame.size.width, height)];
    ...
    self.progress = [[Slider alloc]initWithFrame:CGRectMake(progressX, 0, self.view.frame.size.width - progressX * 2, height)];
    ...
    // 4.添加总时长label
    UILabel *end = [[UILabel alloc]initWithFrame:CGRectMake(self.view.frame.size.width - progressX, 0, 60, height)];
    ...
    // 创建自己的音量条
    UISlider *volume = [[UISlider alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.height / 2.8, 30)]; // 先让slider横放
    volume.center = CGPointMake(40, self.view.frame.size.height / 2);
    ...
    // 水平滑动显示的进度label
    UILabel *horizontalLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height / 1.3, self.view.frame.size.width, 40)];
